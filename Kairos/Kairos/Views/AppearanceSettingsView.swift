...
struct AppearanceSettingsView: View {
    ...
    private var hasUnsavedChanges: Bool {
        mode != savedMode || theme != savedTheme || textColor != savedTextColor || font != savedFont ||
    ...
    var body: some View {
        List {
            Section { Picker("Appearance", selection: $mode) { Text("System").tag("system"); Text("Light").tag("light"); Text("Dark").tag("dark") }.pickerStyle(.navigationLink) } header: { Text("Appearance") }
            Section("Accent") {
                Picker("Theme", selection: $theme) { Text("Kairos Purple").tag("purple"); Text("Ocean Blue").tag("blue"); Text("Forest Green").tag("green") }.pickerStyle(.navigationLink)
                HStack(spacing: 12) {
                    Circle().fill(themeColor.gradient).frame(width: 34, height: 34)
                    VStack(alignment: .leading, spacing: 2) { Text("Kairos accent").font(.subheadline.weight(.semibold)); Text("Updates the app tint and visual atmosphere").font(.caption).foregroundStyle(.secondary) }
                }
            }
            Section("Typography") {
                Picker("Text color", selection: $textColor) { Text("Default").tag("default"); Text("White").tag("white"); Text("Black").tag("black") }.pickerStyle(.navigationLink)
                Picker("Font", selection: $font) { fontRow("System", "system"); fontRow("Rounded", "rounded"); fontRow("Serif", "serif"); fontRow("Monospaced", "monospaced") }.pickerStyle(.navigationLink)
                Text("Aa — The quick brown fox").font(kairosFont(font).weight(.medium)).padding(.vertical, 4)
            }
            Section("Background") {
                Picker("Style", selection: $background) { Text("Gradient").tag("gradient"); Text("Solid").tag("solid"); Text("Minimal").tag("minimal") }.pickerStyle(.navigationLink)
                TextField("Image name or direct image URL (optional)", text: $customBackground).textInputAutocapitalization(.never).autocorrectionDisabled().keyboardType(.URL)
                Text("For bundled images, add the file to Kairos/Resources/Backgrounds and enter its filename without the extension.").font(.caption).foregroundStyle(.secondary)
            }
            Section("Focus") {
                Picker("Ambient sound", selection: $ambientSound) { Text("None").tag("none"); Text("Rain").tag("rain"); Text("Forest").tag("forest"); Text("Ocean").tag("ocean") }.pickerStyle(.navigationLink)
                if ambientSound != "none" {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack { Text("Volume"); Spacer(); Text("\(Int(ambientVolume.rounded()))%").foregroundStyle(.secondary).monospacedDigit() }
                        Slider(value: $ambientVolume, in: 0...100, step: 1)
                    }
                    TextField("Direct audio file URL (optional)", text: $customAmbientURL).textInputAutocapitalization(.never).autocorrectionDisabled().keyboardType(.URL)
                    Text("Built-in sounds are loaded from Kairos/Resources/AmbientSounds as rain, forest, or ocean audio files. The custom URL is retained for future remote playback support.").font(.caption).foregroundStyle(.secondary)
                }
                Toggle("Confetti", isOn: $confetti)
            }
            Section("Accessibility") {
                Toggle("Reduce motion", isOn: $reduceMotion)
                Text("Disables Kairos animated transitions where supported.").font(.caption).foregroundStyle(.secondary)
            }
            Section {
                Button { Task { await save() } } label: { HStack { Text(isSaving ? "Saving…" : "Save appearance"); Spacer(); if isSaving { ProgressView() } } }.disabled(isSaving || !hasUnsavedChanges)
            }
        }
        ...
        .toolbar { ToolbarItem(placement: .topBarLeading) { Button { attemptLeave() } label: { Label("Back", systemImage: "chevron.left") }.disabled(isSaving) } }
    ...
    private func syncSavedValues() {
        savedMode = mode; savedTheme = theme; savedTextColor = textColor; savedFont = font; savedBackground = background; savedCustomBackground = customBackground
        savedAmbientSound = ambientSound; savedAmbientVolume = ambientVolume; savedCustomAmbientURL = customAmbientURL; savedConfetti = confetti; savedReduceMotion = reduceMotion
    }

    private func attemptLeave() { guard hasUnsavedChanges else { dismiss(); return }; showingUnsavedChanges = true }
    ...
    private func persistDraft() async -> Bool {
        ...
        guard profileRepo.accessibilitySettings == accessibility else { profileRepo.appearanceSettings = originalAppearance; profileRepo.accessibilitySettings = originalAccessibility; return false }
        return true
    }
}

#Preview { AppearanceSettingsView().environment(UserProfileRepository()) }
