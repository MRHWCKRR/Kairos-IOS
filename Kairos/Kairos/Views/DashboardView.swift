...
struct DashboardView: View {
        ...
        HStack(alignment: .center, spacing: 12) {
            ...
            Button { showingProfile = true } label: {
                Image(systemName: "person.crop.circle.fill")
                    .font(.system(size: 34)).foregroundStyle(KairosColors.accent).symbolRenderingMode(.hierarchical).frame(width: 44, height: 44)
            }
        ...
        Group {
            if let focusTimer {
                VStack(alignment: .leading, spacing: 18) {
                    HStack {
                        Label("FOCUS TIMER", systemImage: "timer").font(.caption.weight(.bold)).tracking(1).foregroundStyle(KairosColors.accent)
                        Spacer()
                        HStack(spacing: 6) {
                            Circle().fill(focusTimer.isRunning ? KairosColors.accent : .secondary).frame(width: 6, height: 6)
                            Text(focusTimer.isRunning ? "ACTIVE" : "READY").font(.caption2.weight(.bold)).foregroundStyle(focusTimer.isRunning ? KairosColors.accent : .secondary)
                        }
                    }
                    HStack(alignment: .lastTextBaseline) {
                        Text(FocusTimerViewModel.formatHMS(focusTimer.elapsedSeconds)).font(.system(size: 46, weight: .bold, design: .rounded)).monospacedDigit()
                        Spacer()
                        VStack(alignment: .trailing, spacing: 3) {
                            Text("Longest").font(.caption).foregroundStyle(.secondary)
                            Text(FocusTimerViewModel.formatHMS(profileRepo.focusData?.longestSessionSeconds ?? 0)).font(.caption.weight(.semibold))
                        }
                    }
    ...
    private func focusControls(_ timer: FocusTimerViewModel) -> some View {
        HStack(spacing: 10) {
            ...
            Button {
                ...
                Image(systemName: "stop.fill").frame(width: 46, height: 46)
            }
            .buttonStyle(.plain).foregroundStyle(.primary).kairosGlass(cornerRadius: 23)
            .accessibilityLabel("Stop focus timer").accessibilityHint("Stops and logs the current focus session")
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack {
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text(todayTasks.count == 1 ? "One task ready when you are" : "A few things to move forward").font(.caption).foregroundStyle(.secondary)
                    }
                    Spacer()
                    Button("See all") { onNavigate?(1) }.font(.caption.weight(.semibold)).foregroundStyle(KairosColors.accent).kairosGlass(cornerRadius: 16, tint: KairosColors.accent.opacity(0.08))
                }
                VStack(spacing: 0) {
                    ForEach(Array(todayTasks.enumerated()), id: \.element.id) { index, task in
                        HStack(spacing: 12) {
                            Image(systemName: "circle").font(.system(size: 18, weight: .medium)).foregroundStyle(KairosColors.accent)
                            Text(task.title).font(.subheadline.weight(.medium)).lineLimit(2)
                            Spacer(minLength: 8)
                            if task.date == todayKey { Text("Today").font(.caption2.weight(.semibold)).foregroundStyle(.secondary) }
                        }
                        .padding(.vertical, 11)
    ...
    private var goalsCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                Spacer()
                Button { onNavigate?(4) } label: { Image(systemName: "arrow.up.right").font(.caption.weight(.bold)).frame(width: 34, height: 34) }
                    .foregroundStyle(KairosColors.accent).kairosGlass(cornerRadius: 17, tint: KairosColors.accent.opacity(0.08)).accessibilityLabel("Open goals")
            }
    ...
    private var routineCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                Spacer()
                Text("\(Int(completion * 100))%").font(.system(size: 28, weight: .bold, design: .rounded)).foregroundStyle(KairosColors.accent)
            }
            ProgressView(value: completion).tint(KairosColors.accent).scaleEffect(y: 1.5).padding(.vertical, 4)
            HStack {
                Text("\(completedCount) of \(allTasks.count) tasks completed").font(.subheadline.weight(.medium)).foregroundStyle(.secondary)
                Spacer()
                Image(systemName: completion >= 1 ? "checkmark.seal.fill" : "checkmark.circle.fill").foregroundStyle(KairosColors.accent)
            }
        }
        .padding(18)
        .background(.thinMaterial, in: RoundedRectangle(cornerRadius: 26, style: .continuous))
    ...
    private var boardsPreview: some View {
        if planRepo.isLoading {
            HStack { ProgressView(); Text("Loading your plan…").font(.subheadline.weight(.medium)).foregroundStyle(.secondary); Spacer() }
                .padding(20).background(.thinMaterial, in: RoundedRectangle(cornerRadius: 26, style: .continuous))
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack {
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text(boards.isEmpty ? "Nothing here yet" : "A quick look at your work").font(.caption).foregroundStyle(.secondary)
                    }
                    Spacer()
                    Button { onNavigate?(1) } label: { Text("Open").font(.caption.weight(.semibold)) }
                        .foregroundStyle(KairosColors.accent).kairosGlass(cornerRadius: 16, tint: KairosColors.accent.opacity(0.08))
                }
                ForEach(Array(boards.prefix(2))) { board in boardPreview(board) }
                if boards.count > 2 { Text("+\(boards.count - 2) more boards").font(.caption.weight(.medium)).foregroundStyle(.secondary).frame(maxWidth: .infinity, alignment: .center).padding(.top, 2) }
            }
            ...
            VStack(spacing: 10) {
                ...
                Text("Create a plan on Kairos web or Android to see your boards here.").font(.subheadline).foregroundStyle(.secondary).multilineTextAlignment(.center)
                Button("Open Boards") { onNavigate?(1) }.font(.subheadline.weight(.semibold)).foregroundStyle(KairosColors.accent).padding(.top, 2)
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(Array(board.sections.filter { !$0.archived }.prefix(2))) { section in
                ...
                HStack(spacing: 9) {
                    ...
                    Spacer()
                    Text("\(visibleTasks.filter(\.completed).count)/\(visibleTasks.count)").font(.caption2.monospacedDigit().weight(.semibold)).foregroundStyle(.secondary)
                }
            }
        }
