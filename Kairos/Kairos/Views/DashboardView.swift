...
struct DashboardView: View {
    ...
    private var todayTasks: [KairosTask] {
        ...
        return Array((scheduled.isEmpty ? activeTasks.filter { $0.date == nil } : scheduled).prefix(5))
    }
    ...
    private var focusCard: some View {
            ...
            if let focusTimer {
                VStack(alignment: .leading, spacing: 18) {
                    HStack {
                        ...
                        Spacer()
                        Circle()
                            .fill(focusTimer.isRunning ? accent : .secondary.opacity(0.45))
                    ...
                    HStack(alignment: .bottom) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(FocusTimerViewModel.formatHMS(focusTimer.elapsedSeconds))
                                ...
                                .monospacedDigit()
                                .tracking(-1.5)
                            Text(focusTimer.isRunning ? "Stay in the zone." : "Ready when you are.")
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                        }
                        Spacer()
                        HStack(spacing: 10) {
                            ...
                            if focusTimer.isRunning {
                                Button { focusTimer.stopAndLog(); fireFocusHaptic() } label: {
                                    Image(systemName: "stop.fill")
                                        .font(.system(size: 13, weight: .semibold))
                                        .frame(width: 42, height: 42)
                                }
                                .foregroundStyle(.primary)
                                .kairosGlass(cornerRadius: 21)
        ...
        .animation(reduceMotion ? nil : .spring(response: 0.35, dampingFraction: 0.84), value: focusTimer?.isRunning)
    }
        ...
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(todayTasks.isEmpty ? "Your day has some breathing room." : "A short list, a clear head.")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                ...
                VStack(spacing: 0) {
                    ForEach(Array(todayTasks.enumerated()), id: \.element.id) { index, task in
                        ...
                        if index < todayTasks.count - 1 {
                            Divider().padding(.leading, 38).opacity(0.28)
                        }
                    }
                }
            }
    ...
    private func taskLocation(for task: KairosTask) -> (boardID: String, sectionID: String)? {
        ...
        for board in plan.boards where !board.archived {
            for section in board.sections where !section.archived where section.tasks.contains(where: { $0.id == task.id }) {
    ...
    private func taskRow(_ task: KairosTask) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 13) {
                ZStack {
                    Circle().stroke(accent.opacity(0.72), lineWidth: 1.7).frame(width: 21, height: 21)
                    if isCompleting {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(task.title)
                        ...
                        .strikethrough(isCompleting, color: accent)
                        .lineLimit(2)
    ...
    private var progressCard: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack {
                ...
                .foregroundStyle(accent)
                .kairosGlass(cornerRadius: 17, tint: accent.opacity(0.08))
                .accessibilityLabel("Open goals")
            }

            HStack(spacing: 12) {
                metric(value: "\(Int(completion * 100))%", label: "Routine")
                metric(value: "\(completedCount)", label: "Completed")
                metric(value: "\(profileRepo.achievementsData?.unlocked?.count ?? 0)", label: "Unlocked")
            }

            ProgressView(value: completion)
                .tint(accent)
                .scaleEffect(y: 1.35)
                .animation(reduceMotion ? nil : .easeInOut(duration: 0.45), value: completion)
        }
        .padding(20)
        .kairosCard(cornerRadius: 28)
    }
    ...
    private var quickActions: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 12) {
                quickAction(title: "Boards", icon: "square.stack.3d.up.fill", tab: 1)
                quickAction(title: "Calendar", icon: "calendar", tab: 3)
                quickAction(title: "Goals", icon: "trophy.fill", tab: 4)
            }
        }
    }

    private func quickAction(title: String, icon: String, tab: Int) -> some View {
        Button { onNavigate?(tab) } label: {
            ...
            .padding(.vertical, 14)
            .kairosGlass(cornerRadius: 20, tint: accent.opacity(0.04))
        }
        .buttonStyle(.plain)
    }
