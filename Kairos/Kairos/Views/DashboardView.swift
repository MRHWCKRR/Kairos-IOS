            ...
            if let focusTimer {
                VStack(alignment: .leading, spacing: 0) {
                    HStack(alignment: .center) {
                        ...
                        HStack(spacing: 7) {
                            Circle()
                                .fill(focusTimer.isRunning ? KairosColors.accent : .secondary.opacity(0.6))
                                .frame(width: 7, height: 7)

                            Text(focusTimer.isRunning ? "In session" : "Ready")
                        ...
                        if let longest = profileRepo.focusData?.longestSessionSeconds, longest > 0 {
                            VStack(alignment: .trailing, spacing: 3) {
                                Text("Best session")
                                    .font(.caption2)
                                    .foregroundStyle(.secondary)
                                Text(FocusTimerViewModel.formatHMS(longest))
                                    .font(.caption.weight(.semibold))
                            }
                        }
                    }
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            if todayTasks.isEmpty {
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    Text("No unfinished tasks are scheduled. A little space can be productive too.")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
        ...
        VStack(alignment: .leading, spacing: 22) {
            ...
            HStack(spacing: 18) {
                progressStat(
                    ...
                    title: "Routine",
                    subtitle: allTasks.isEmpty ? "No tasks yet" : "\(completedCount) of \(allTasks.count) tasks"
                )
        ...
        Button {
            ...
            HStack(spacing: 14) {
                Image(systemName: "square.stack.3d.up.fill")
                    ...
                    .background(KairosColors.accent.opacity(0.10), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
