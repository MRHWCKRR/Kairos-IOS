...
final class StudyPlanRepository {
    ...
    private func mutateBoards(_ transform: (inout [KairosBoard]) -> Void) async {
        ...
        transform(&boards)
        if let plan = currentPlan { var optimisticPlan = plan; optimisticPlan.boards = boards; currentPlan = optimisticPlan }
        else { currentPlan = KairosStudyPlan(id: nil, userID: userID, createdAt: nil, boards: boards, dayInsights: nil, scheduleEvents: nil) }
        do {
            let encoder = Firestore.Encoder()
    ...
    func renameBoard(boardID: String, title: String) async { await updateBoard(boardID: boardID, title: title, description: nil, preserveDescription: true) }
    func updateBoard(boardID: String, title: String? = nil, description: String? = nil, preserveDescription: Bool = false) async {
        await mutateBoards { boards in
            guard let i = boards.firstIndex(where: { $0.id == boardID }) else { return }
            if let title { boards[i].title = title }
            if description != nil || !preserveDescription { boards[i].boardDescription = description }
        }
    }
    ...
    // MARK: - Sections
    func addSection(boardID: String, title: String) async { await mutateBoards { boards in guard let bi = boards.firstIndex(where: { $0.id == boardID }) else { return }; boards[bi].sections.append(KairosSection(id: "sec-\(UUID().uuidString)", title: title, archived: false, tasks: [])) } }
    ...
    // MARK: - Tasks
    func addTask(sectionID: String, title: String) async { await mutateBoards { boards in for bi in boards.indices { if let si = boards[bi].sections.firstIndex(where: { $0.id == sectionID }) { boards[bi].sections[si].tasks.append(KairosTask(id: "task-\(UUID().uuidString)", title: title, completed: false, archived: false, date: nil)); return } } } }
    func addSubtask(parentTaskID: String, sectionID: String, title: String) async { await mutateBoards { boards in for bi in boards.indices { guard let si = boards[bi].sections.firstIndex(where: { $0.id == sectionID }) else { continue }; boards[bi].sections[si].tasks.append(KairosTask(id: "task-\(UUID().uuidString)", title: title, completed: false, archived: false, date: nil, taskDescription: nil, dueTime: nil, parentTaskID: parentTaskID)); return } } }
    func updateTask(taskID: String, title: String? = nil, description: String? = nil, date: String? = nil, dueTime: String? = nil, parentTaskID: String? = nil) async { await mutateBoards { boards in for bi in boards.indices { for si in boards[bi].sections.indices { if let ti = boards[bi].sections[si].tasks.firstIndex(where: { $0.id == taskID }) { if let title { boards[bi].sections[si].tasks[ti].title = title }; boards[bi].sections[si].tasks[ti].taskDescription = description; boards[bi].sections[si].tasks[ti].date = date; boards[bi].sections[si].tasks[ti].dueTime = dueTime; if parentTaskID != nil { boards[bi].sections[si].tasks[ti].parentTaskID = parentTaskID }; return } } } } }
    ...
    func applyAiPlan(sections: [KairosSection], recurringEvents: [KairosScheduleEvent], newBoardTitle: String?, existingBoardID: String?) async {
        ...
        else { let trimmed = newBoardTitle?.trimmingCharacters(in: .whitespacesAndNewlines); let title = (trimmed?.isEmpty == false) ? trimmed! : "AI Plan"; boards.append(KairosBoard(id: "board-\(UUID().uuidString)", title: title, archived: false, sections: sections)) }
        ...
        do { let encoder = Firestore.Encoder(); let encodedBoards = try boards.map { try encoder.encode($0) }; let encodedEvents = try events.map { try encoder.encode($0) }; if let planID = previousPlan?.id { try await db.collection("study_plans").document(planID).updateData(["boards": encodedBoards, "scheduleEvents": encodedEvents]) } else { let reference = try await db.collection("study_plans").addDocument(data: ["boards": encodedBoards, "scheduleEvents": encodedEvents, "userID": userID, "createdAt": FieldValue.serverTimestamp()]); currentPlan?.id = reference.documentID }; errorMessage = nil } catch { currentPlan = previousPlan; errorMessage = "Failed to apply AI plan: \(error.localizedDescription)" }
    }
}